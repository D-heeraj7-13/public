...
export async function POST(req: Request) {
  try {
    ...
    // Resolve auth token from env, header, or body
    let authToken =
      process.env.token ||
    ...
    // Find the specific memory utilization item
    const targetMemory = items.find((item: any) => item.key_ === "vm.memory.util[memoryUsedPercentage.0]");
    
    // Get history for the target memory item or fallback to first item
    const itemsToProcess = targetMemory ? [targetMemory] : items.slice(0, 1);

    // Get history for each item (last hour)
