...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const user_token = response.data.result;
      console.log("user_token", response.data.result)
      if (data.result) {
        const token = data.result;
        localStorage.setItem("zabbix_auth", token);
        router.push("/dashboard");
      } else {
        const err = data.error?.data || data.error || "Unknown error";
      ...
      setError("Connection error: Could not reach Zabbix API.");
    } finally {
      setLoading(false);
    }
  };
      ...
      >
        ...
        <div>
          ...
          <input
            ...
            className="w-full border border-blue-200 bg-white/70 rounded-lg px-3 py-2 
        ...
        <button
          ...
          className="w-full bg-blue-600 text-white py-2 rounded-lg 
            hover:bg-blue-700 shadow-lg transition-all disabled:opacity-50"
        >
