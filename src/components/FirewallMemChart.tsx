...
const Tooltip = ({ payload, active, label }: TooltipProps) => {
  ...
  // Only show the first item (single mem)
...
export function FirewallMemChart({ initialData, initialCategories }: { initialData?: FormattedDataPoint[]; initialCategories?: string[] }) {
  ...
  const fetchmemData = async () => {
    ...
    try {
      const res = await fetch("/api/zabbix/memory", {
        method: "POST",
      });
      ...
      if (!response?.result?.history) {
        console.warn("No memory history data received");
        setLoading(false);
        return;
      }

      const historyData: memItem[] = response.result.history;

      // Filter for only the specific memory utilization key
      const targetMemory = historyData.find(
        (item) => item.key === "vm.memory.util[memoryUsedPercentage.0]"
      );

      if (!targetMemory) {
        console.warn("No data found for vm.memory.util[memoryUsedPercentage.0]");
      ...
      if (targetMemory.history.length === 0) {
        console.warn("No history data found for vm.memory.util[memoryUsedPercentage.0]");
        setLoading(false);
        setChartData([]);
        return;
      }
      ...
      const formattedData: FormattedDataPoint[] = targetMemory.history.map((point) => ({
      ...
      setCategories(["Memory Utilization"]);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(fetchmemData, 60000); // Refresh every 1 minute
